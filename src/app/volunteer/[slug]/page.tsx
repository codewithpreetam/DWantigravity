...
import { ArrowLeft, ArrowUpRight, Building, MapPin, Clock, AlertCircle } from "lucide-react";
...
export default async function VolunteerDetailPage(props: Props) {
  const params = await props.params;
  const session = await auth();
  const user = session?.user;
  let volunteer = await db.volunteer.findUnique({
    where: { slug: params.slug },
  });

  if (!volunteer) {
    if (params.slug.length >= 25 && params.slug.startsWith("c")) {
      volunteer = await db.volunteer.findUnique({
        where: { id: params.slug },
      });
      if (volunteer) {
        redirect(`/volunteer/${volunteer.slug}`);
      }
    }
    return notFound();
  }
  ...
  // Resolve relations
  const organization = await db.organization.findUnique({ where: { id: volunteer.organizationId } });
  
  const teamMembers = volunteer.organizationId 
    ? await db.teamMember.findMany({
        where: { organizationId: volunteer.organizationId },
  ...
  const isSaved = user?.id
    ? !!(await db.savedJob.findFirst({
        where: { candidateId: user.id, volunteerId: volunteer.id },
      }))
    : false;
  ...
  const isPastDeadline = volunteer.deadline 
    ? new Date() > new Date(new Date(volunteer.deadline).setUTCHours(23, 59, 59, 999)) 
    : false;
  const isActive = volunteer.isActive !== false; // Default to true if undefined
  const isClosed = isPastDeadline || !isActive;

  return (
    <div className="max-w-4xl mx-auto px-4 py-12 flex-1 space-y-6 text-left">
      <Link href="/volunteer" className="text-xs text-primary hover:underline flex items-center gap-1">
      ...
      <div className="glass-panel p-5 sm:p-8 rounded-2xl border border-card-border space-y-6">
        ...
        <div className="border-t border-card-border pt-4">
          <h3 className="font-bold text-foreground text-xs uppercase tracking-wider mb-2">Campaign Description</h3>
          <div className="text-xs text-muted leading-relaxed whitespace-pre-wrap prose prose-sm max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: volunteer.description }} />
        </div>

        {volunteer.requirements && (
          <div className="border-t border-card-border pt-4">
            <h3 className="font-bold text-foreground text-xs uppercase tracking-wider mb-2">Skills Needed / Requirements</h3>
            <p className="text-xs text-muted leading-relaxed whitespace-pre-line">{volunteer.requirements}</p>
          </div>
        )}
        ...
        <div className="border-t border-card-border pt-6 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto items-center">
            ...
            {isClosed ? (
              ...
              <ApplyButton opportunityId={volunteer.id} opportunityTitle={volunteer.title} opportunityType="VOLUNTEER" userEmail={user.email || undefined} label="Apply Now" alreadyApplied={!!alreadyApplied} />
            ) : (
              <Link href={`/auth/signin?callbackUrl=/volunteer/${volunteer.slug}`} className="px-5 py-2 bg-primary hover:bg-primary-hover text-white text-xs font-semibold rounded-lg w-full sm:w-auto text-center">Login to Apply</Link>
            )}
          </div>
        </div>
      </div>
