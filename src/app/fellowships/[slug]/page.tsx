...
export default async function FellowshipDetailPage(props: Props) {
  ...
  let fellowship = await db.fellowship.findUnique({
    where: { slug: params.slug },
  });

  if (!fellowship) {
    if (params.slug.length >= 25 && params.slug.startsWith("c")) {
      fellowship = await db.fellowship.findUnique({
        where: { id: params.slug },
      });
      if (fellowship) {
        redirect(`/fellowships/${fellowship.slug}`);
      }
    }
    return notFound();
  }
  ...
  const teamMembers = fellowship.organizationId 
    ? await db.teamMember.findMany({
        where: { organizationId: fellowship.organizationId },
  ...
  const isSaved = user?.id
    ? !!(await db.savedJob.findFirst({
        where: { candidateId: user.id, fellowshipId: fellowship.id },
      }))
    : false;
  ...
  const isPastDeadline = fellowship.deadline 
    ? new Date() > new Date(new Date(fellowship.deadline).setUTCHours(23, 59, 59, 999)) 
    : false;
  const isActive = fellowship.isActive !== false; // Default to true if undefined
        ...
        <div className="border-t border-card-border pt-6 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto items-center">
            ...
            {isClosed ? (
              ...
              <ApplyButton opportunityId={fellowship.id} opportunityTitle={fellowship.title} opportunityType="FELLOWSHIP" userEmail={user.email || undefined} label="Apply Now" alreadyApplied={!!alreadyApplied} />
            ) : (
              <Link href={`/auth/signin?callbackUrl=/fellowships/${fellowship.slug}`} className="px-5 py-2 bg-primary hover:bg-primary-hover text-white text-xs font-semibold rounded-lg w-full sm:w-auto text-center">Login to Apply</Link>
            )}
          </div>
        </div>
      </div>
