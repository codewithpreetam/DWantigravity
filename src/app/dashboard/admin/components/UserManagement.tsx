...
export default function UserManagement() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // If none selected, fetch all filtered data or use current page.
      // Usually, we fetch all matching if we want to export filtered. For now, export current table view or fetch all.
      // We'll just export the selected ones or all fetched if none selected.
      if (selectedIds.length > 0) {
        exportData = users.filter(u => selectedIds.includes(u.id));
      } else {
        // Fetch ALL matching the filter without pagination for full export
      ...
      const formatted = exportData.map(u => ({
        ...
        "Registration Date": format(new Date(u.createdAt), "PPP"),
        "Opportunities Posted": u.totalOpportunities,
                  ...
                  <tr key={user.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        {user.role === 'ADMIN' ? <Shield className="w-3.5 h-3.5 text-primary" /> : user.role === 'EMPLOYER' ? <Briefcase className="w-3.5 h-3.5 text-amber-500" /> : <UserIcon className="w-3.5 h-3.5 text-secondary" />}
                        <span className="font-medium text-sm capitalize">{user.role.toLowerCase()}</span>
                      </div>
                      {user.role === 'EMPLOYER' && <div className="text-xs text-muted-foreground mt-0.5">{user.organization}</div>}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right">
                      <div className="text-sm font-medium text-foreground">{user.role === 'EMPLOYER' ? `${user.totalOpportunities} Posts` : `${user.totalApplications} Apps`}</div>
                    </td>
                  </tr>
                ))
              )}
